...
import {
  ReminderFiredBanner,
  ReminderAckedBanner,
} from "../components/ui/ReminderBanner";
...
function AppNavigator() {
  ...
  const { pending, count, addFromWs, markAcked, reload, isLoading } =
    usePendingAlerts();
  useEffect(() => {
    loadInvites();
  }, [loadInvites]);
  const { wsRef } = useWebSocket((message) => {
    ...
    if (message.event === "reminder_acked") {
      ...
      setFired((current) =>
        current?.reminder_id === payload.reminder_id ? null : current,
      );
      setAcked(payload);
    }
  });
  ...
  const page =
    active === "overview" ? (
      ...
      <FamiliesNavigator
        liveInvites={invites}
        onInviteResponded={removeInvite}
      />
    ) : (
      <ProfileScreen />
    );
  return (
    <View style={styles.app}>
      ...
      {acked ? (
        <ReminderAckedBanner payload={acked} onDismiss={() => setAcked(null)} />
      ) : null}
