  ...
  async function handleAdd() {
    ...
    try {
      ...
      if ((res as any).type === 'success' && (res as any).uri) {
        ...
        fileMime =
          (res as any).mimeType || (fileName?.endsWith('.pdf') ? 'application/pdf' : undefined);
      ...
      try {
        const uploaded = await uploadApi.petDocument(
          petId,
          fileUri,
        ...
        showAlert(
          uploadErr instanceof Error
            ? uploadErr.message
            : t('upload_failed', 'Caricamento fallito'),
          { type: 'error' },
        );
      }
    } catch (e) {
      try {
        ...
        showAlert(
          imgErr instanceof Error ? imgErr.message : t('upload_failed', 'Caricamento fallito'),
          { type: 'error' },
        );
      }
    } finally {
        ...
        localDocs.map((d) => (
          <View key={d.id ?? d.url} style={styles.row}>
            <TouchableOpacity style={styles.metaTouch} onPress={() => openDoc(d)}>
              {d.mime_type?.startsWith('image/') ? <DocumentThumb d={d} urlMap={urlMap} /> : null}
